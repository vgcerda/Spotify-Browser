...
export class SpotifyService {
  ...
  searchFor(category:string, resource:string):Promise<ResourceData[]> {
    ...
    return this.sendRequestToExpress('/search/' + category + '/' + encodeURIComponent(resource)).then((data) => {
      // console.log(data);
      let resourceData:ResourceData[] = null;
      if (category === 'artist') {
        resourceData = data["artists"]["items"].map((item)=>{
        ...
        resourceData = data["tracks"]["items"].map((item)=>{
        ...
        resourceData = data["albums"]["items"].map((item)=>{
  ...
  getRelatedArtists(artistId:string):Promise<ArtistData[]> {
    ...
    return this.sendRequestToExpress('/artist-related-artists/' + encodeURIComponent(artistId)).then((data) => {
      let resourceData:ArtistData[] = null;
      resourceData=data["artists"].map((item) => {
        return new ArtistData(item);
      });
      return resourceData;
    });
  ...
  getTopTracksForArtist(artistId:string):Promise<TrackData[]> {
    ...
    return this.sendRequestToExpress('/artist-top-tracks/' + encodeURIComponent(artistId)).then((data) => {
      let resourceData:TrackData[] = null;
      resourceData =data["tracks"].map((item) =>{
        return new TrackData(item);
      });
      return resourceData;
    });
  ...
  getAlbumsForArtist(artistId:string):Promise<AlbumData[]> {
    ...
    return this.sendRequestToExpress('/artist-albums/' + encodeURIComponent(artistId)).then((data) => {
      let resourceData:AlbumData[] = null;
      resourceData =data["items"].map((item) =>{
        return new AlbumData(item);
      });
      return resourceData;
    });
  ...
  getAudioFeaturesForTrack(trackId:string):Promise<TrackFeature[]> {
    ...
    return this.sendRequestToExpress('/track-audio-features/' + encodeURIComponent(trackId)).then((data) => {
      let resourceData:TrackFeature[] = [];
      console.log(data);
      for (let i = 0; i < TrackFeature.FeatureTypes.length; i++){
        let trackFeatureString = TrackFeature.FeatureTypes[i];
        resourceData.push(new TrackFeature(trackFeatureString, data[trackFeatureString]));
      }
      return resourceData;
    });
  }
